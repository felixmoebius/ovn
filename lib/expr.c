...
static int
contains_expr_constant_integer_cb(const void *a_, const void *b_)
{
    const struct expr_constant *a = a_;
    const struct expr_constant *b = b_;
    union mf_subvalue d_value, d_mask;
    ...
    b_mask = b->masked ? &b->mask : &exact_sub_match_mask;
    if (!mf_subvalue_intersect(&a->value, &a->mask, &b->value,
                               b_mask, &d_value, &d_mask)) {
        return 1;
    }

    return memcmp(b_mask, &d_mask, sizeof *b_mask);
}
 ...
 * The resulting set is sorted and has duplicates removed such that it may
 * be used as input to expr_constant_set_integers_diff.
...
expr_constant_set_create_integers(const char *const *values, size_t n_values)
{
    ...
    /* Sort and deduplicate the result, so that it is possible to generate diffs
     * in the function expr_constant_set_diff */
    vector_qsort(&cs->values, compare_expr_constant_integer_cb);
    vector_dedup(&cs->values, contains_expr_constant_integer_cb);
 ...
 * type, must be sorted and must not contain duplicate values (which is true
 * if they are generated by expr_constant_set_create_integers() or
